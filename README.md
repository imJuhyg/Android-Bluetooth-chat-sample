...
* 서버 소켓에 연결 요청을 보내는 connect() 메소드와 상대방 소켓으로부터 보내온 메시지를 읽는 read() 메소드는 메인 스레드를 중단시킵니다. 안드로이드의 메인 스레드는 오직 UI 업데이트에 의해서만 사용되어야 하기 때문에 새로운 스레드에서 작업해야 합니다. 멀티스레딩을 구현하는 방식은 여러 가지가 있지만 예제에서는 Thread/Handler를 통해 구현했습니다.
